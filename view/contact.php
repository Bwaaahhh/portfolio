<div id="contact">
    <div class="row">
        <form class="" action="index.html" method="post">
            <p class="large-4 large-centered columns end" id="intro">Bonjour Cyril,</p>
            <div class="large-12 columns end" id="formPremiere">
                <p class="">Je suis </p>
                <input class="inputForm"type="text" name="nomPerso" value="nom & Prenom"><i class="fa fa-pencil" aria-hidden="true"></i>
                <p>et je vous contacte de la </p>
            </div>
            <div class="" id="formDeuxieme">
                <p>part de</p>
                <input class="inputForm" type="text" name="nomSociete" value="nom de la société"><i class="fa fa-pencil" aria-hidden="true"></i>
                <p>pour discuter</p>
            </div>
            <div class="" id="formTroisieme">
                <p>d'un(e)</p>
                <select class="selectForm" name="offre">
                    <option value="">projet</option>
                    <option value="">offre d'emploi</option>
                    <option value="">entretien</option>
                    <option value="">... c'est secret</option>
                </select>.
            </div>
            <p>Merci d'avance de bien vouloir me répondre</p>
            <div class="" id="formQuatre">
                <p>à</p>
                <input class="inputForm" type="text" name="mail" value="mon adress e-mail">
                <i class="fa fa-pencil" aria-hidden="true"></i>
                <p>ou bien contactez-moi</p>
            </div>
            <div class="" id="formCinq">
                <p>simpement par téléphone au</p>
                <input class="inputForm" type="text" name="phone" value="+33">
                <i class="fa fa-pencil" aria-hidden="true"></i>
            </div>
            <div class="row bye">
                <p class="large-2 large-centered columns">À bientot</p>
            </div>
            <div class="row">
                <p class="large-2 large-centered columns">//</p>
            </div>
            <div class="row" id="formButton">
                <button class="large-3 columns large-offset-2" id="resetForm">réinitialiser le message</button>
                <button class="large-3 large-offset-2 columns end ">envoyer le message</button>
            </div>
        </form>
    </div>
</div>
